...
public class VentanaAddGrupo extends JDialog implements ActionListener {
	...
	public VentanaAddGrupo(JFrame parent) {
		...
		contactosIndividuales = Controlador
				.getContactosIndividuales(Controlador.INSTANCE.getContactosOrdenadosAlfabeticamente());
		...
		for (ContactoIndividual ci : contactosIndividuales) {
			...
			panelCheckList.add(Box.createVerticalStrut(1));
		}
		...
		if (src == botonElegirImagen) {
			...
			if (!files.isEmpty()) {
				try {
					...
					JOptionPane.showMessageDialog(this, "No se pudo cargar la imagen.", "Error",
			...
			if (nombreGrupo.isEmpty()) {
				JOptionPane.showMessageDialog(this, "Debe introducir un nombre para el grupo.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			if (seleccionados.isEmpty()) {
				JOptionPane.showMessageDialog(this, "Debe seleccionar al menos un contacto.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			if (rutaImagenSeleccionada == null) {
				JOptionPane.showMessageDialog(this, "Debe seleccionar una imagen para el grupo.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "No se pudo guardar la imagen.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			if (Controlador.INSTANCE.registrarGrupo(nombreGrupo, rutaImagen, seleccionados)) {
				JOptionPane.showMessageDialog(this, "Grupo creado correctamente.", "Éxito",
						JOptionPane.INFORMATION_MESSAGE);
			} else {
				JOptionPane.showMessageDialog(this, "Ya existe un grupo con ese nombre.", "Error",
						JOptionPane.ERROR_MESSAGE);
				return;
			}
			dispose();
